...
def _create_questions_list(sql: str, filename: str):
...
def _connect_openai(filename: str,promots: list, questions: list, ):
...
def _save_reponses(messages: dict):
...
def main():
    ...
    sql_string = _read_blob(filename)
    messages,questions = _create_questions_list(sql_string, filename)

    response = _connect_openai(filename, messages,questions)

    _save_reponses(response)
